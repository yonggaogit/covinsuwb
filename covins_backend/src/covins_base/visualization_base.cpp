    ...
    : topic_prefix_(topic_prefix)
{
    ...
    std::string traj_topic = "covins_traj_";
    for( int i = 0; i < 5; i++ ) {
        std::string traj_topic_i = traj_topic + std::to_string( i + 1 );
        ros::Publisher pub_traj = nh_.advertise<nav_msgs::Path>( traj_topic_i, 10 );
        traj_pubs_.push_back( pub_traj );
    }
