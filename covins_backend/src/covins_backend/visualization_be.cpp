...
auto Visualizer::PubTrajectories()->void {
    ...
    std::map<int,visualization_msgs::Marker> msgs;
    std::map<int,nav_msgs::Path> msgs_nav;

    int num_cid = kfcids.size();
    for(std::set<int>::iterator sit = kfcids.begin(); sit != kfcids.end(); ++sit){
        int cid = *sit;
        visualization_msgs::Marker traj;
        nav_msgs::Path nav_traj;
        traj.header.frame_id = curr_bundle_.frame;
        traj.header.stamp = ros::Time::now();
        nav_traj.header.frame_id = curr_bundle_.frame;
        nav_traj.header.stamp = ros::Time::now();
    ...
    for(KeyframeSetById::iterator sit = kftraj.begin(); sit != kftraj.end(); ++sit){
        ...
        this_pose_stamped.pose.position.x = T(0, 3);
        this_pose_stamped.pose.position.y = T(1, 3);
    ...
    for(std::map<int,nav_msgs::Path>::iterator mit = msgs_nav.begin(); mit != msgs_nav.end(); ++mit){
        int cid = mit->first;
        nav_msgs::Path msg = mit->second;
        traj_pubs_[cid].publish( msg );
    }
}
